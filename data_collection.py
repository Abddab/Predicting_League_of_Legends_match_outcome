...
def getSummoner(summonerName: str, field: str):
    """
    Retrieves a summonner's information based on a specific field. 
    It is used to fetch a summoner's match data in another API call
...
def getRankedMatchList(summonerPuuid: str, count: int = 20):
    """
    Returns a list of RANKED match ids. It is used as an input to fetch
    each match's details in another API call.
...
def selectRandomPlayer(puuid: list):
    """
    Selects a random summoner's puuid from a provided list of summoners puuids 
    """
...
"""
# The puiid chosen to start the chain of API calls
